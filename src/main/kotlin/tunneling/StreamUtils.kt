 ...
 */
suspend fun readFully(input: InputStream, buffer: ByteArray, timeoutMillis: Long? = null, logPrefix: String = ""): Int {
    val startTime = System.currentTimeMillis()
    if (logPrefix.isNotEmpty()) {
        println("[$logPrefix] Starting read of ${buffer.size} bytes${if (timeoutMillis != null) " with ${timeoutMillis}ms timeout" else ""}")
    }
    
    val bytesRead = if (timeoutMillis != null) {
...
private suspend fun readFullyBlocking(input: InputStream, buffer: ByteArray): Int =
    withContext(Dispatchers.IO) {
        readFullyBlockingSync(input, buffer)
    }

private fun readFullyBlockingSync(input: InputStream, buffer: ByteArray): Int {
    var offset = 0
