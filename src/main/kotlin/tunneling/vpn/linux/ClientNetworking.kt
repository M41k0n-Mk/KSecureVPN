...
object ClientNetworking {
    ...
    private fun runSafe(cmd: List<String>, desc: String, continueOnFail: Boolean = false): Int {
        return try {
            ...
            if (exit != 0) {
                ...
                if (out.isNotBlank()) logger.logSessionEvent("client-net", LogLevel.DEBUG, "$desc ok: ${out.trim()}")
                else logger.logSessionEvent("client-net", LogLevel.INFO, "$desc ok")
            }
            exit
        } catch (e: Exception) {
